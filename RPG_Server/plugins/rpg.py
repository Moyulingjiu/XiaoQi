...
class Core:
                    ...
                    if len(data) > 3:  # 至少得有名字、简介、类型
                        if not self.goods.__contains__(data[0]):
                            ...
                            for j in data:
                                j_list = j.split('=')
                                if len(j_list) == 2:
                                    if j_list[0] != 'comments' and j_list[0] != 'occupation' and goods.__contains__(
                                            j_list[0]):
    ...
    def get_fight_number(self, user):
        fight_number = (self.get_attack(user) + self.get_armor(user)) * self.get_speed(user) * self.get_max_hp(
            user) / 10000
    ...
    def update(self, qq, new_user, result, force: bool = False):
        ...
        # 有生命值减少
        if not new_user['config']['die_check'] and new_user['attribute']['own']['hp']['number'] < \
                user['attribute']['own']['hp']['number']:
        ...
        elif not new_user['config']['die_check'] and new_user['attribute']['own']['hp']['number'] > \
                user['attribute']['own']['hp']['number']:
    ...
    def PVP2(self, active, passive):
        ...
        if active == passive:  # 和自己击剑
            ...
            PVP2['hp_cost'] = hurt2
            PVP2['hp_get'] = hurt1
            report: str = user_passive['config']['report']
            message: str = '你被' + str(active) + '袭击了'
            if message not in report:
                user_passive['config']['report'] += message + '\n'
                        ...
                        if goods['type'] == 27:  # 树苗、种子、牲畜
                            ...
                            if name == '福袋' and number <= 20:
                                ...
                                if name == '姜汤':
                                    ...
                                    use['comments'] = '冥冥之中，你的命运之弦仿佛被拨动了。但是好像什么也没有发生。'
                                elif name == '金苹果':
                                    buff = {'name': '回复', 'type': 1, 'level': random.randint(1, 7),
                                            'date': addition_minute(getNow.toString(), 2),
                                            'times': 0}
                                    reply, user = self.get_buff(user, buff)
                                    buff2 = {'name': '迅捷', 'type': 1, 'level': random.randint(1, 3),
                                             'date': addition_minute(getNow.toString(), 2),
                                             'times': 0}
                                    reply, user = self.get_buff(user, buff2)
                                    use['comments'] += '你获得了2分钟的%d级回复buff，你获得了2分钟的%d级迅捷buff，并移除了虚弱' % (
                                        buff['level'], buff2['level'])
    ...
    def handle(self, message, qq, member_name, bot_name, be_at, limit):
        ...
        if not need_reply:
            if message[:2] == '转职' and not limit:
                ...
                if items['enchanting']['sharp'] != 0 or items['enchanting']['rapid'] != 0 or items['enchanting'][
                    'strong'] != 0:
